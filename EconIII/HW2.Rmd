...
The final model will be given by:
...
$$u^*{t} =  \alpha + \phi_1 u{t-1} + \phi_2 u_{t-3} + \beta gdp_{t-1} + \varepsilon_{t} = u_t$$
...
$$\begin{aligned}
    u^{t+1} &= \alpha + \phi_1 u{t}^ + \phi_2 u_{t-2} + \beta (gdp_{t}+1) + \varepsilon_{t+1} \\
     &=  \alpha + \phi_1 u_{t} + \phi_2 u_{t-2} + \beta gdp_{t} + \beta + \varepsilon_{t+1} \\
     &= u_{t+1} + \beta \\
    u^{t+2} &=  \alpha + \phi_1 u{t+1}^ + \phi_2 u_{t-1} + \beta gdp_{t+1} + \varepsilon_{t+2} \\
...
```{r, message = FALSE, warning = FALSE, echo = FALSE} 
  ...
  p1 <- hist(mRes1, density=20, breaks=20, prob=TRUE, xlab = "Model residuals", 
             ylim=c(0,5), main = "Residuals of ADL(3,1)")
  curve(dnorm(x, mean=mean(mRes1), sd=sigma_Res1), 
...
To compute these probabilities, we first find the one step ahead estimator for $UN\_RATE$. Let the unemployment rate be $u_t$, the gdp quartely growth be $gdp_t$ and the one step ahead forecast be $\hat{u}_{T+1}$. Since we are assuming $\{\varepsilon_t\} \sim iid(0,\sigma_{\varepsilon}^2)$, we have that:
...
Therefore, we have that $e_T \sim iid(0,\sigma_{\varepsilon}^2)$.
...
for (i in indexts:nrow(tsData22)){
  ADL22_forecast[i,2] <- coef(mADL31_0) %*% c(1, ADL22_forecast[(i-1), 2], 
                                              ADL22_forecast[(i-3), 2], ADL22_forecast[(i-1), 1])
}
```
