...
%       Bbound     double, the bounds for the bins - not equidistant
% 
%   Output:
%
...
function Y=tauchenPis(Gridp, Bbound, p, n)

    P = zeros(n,n);
    for i=1:n
        j=1; 
            P(i,j) = normcdf((Bbound(j,1)-p.rho_z*Gridp(i,1))/p.beta_eps);
        j=n; 
            P(i,j) = 1- normcdf((Bbound(j-1,1)-p.rho_z*Gridp(i,1))/p.beta_eps);
        for j=2:n-1
            P(i,j) = normcdf((Bbound(j,1)-p.rho_z*Gridp(i,1))/p.beta_eps) - normcdf((Bbound(j-1,1)-p.rho_z*Gridp(i,1))/p.beta_eps);
        end
    end
    % check if row-sum = 1
